/// Royal theme — mirrors `src/templates/royal/index.jsx`'s section order:
/// Navbar → Hero → Countdown → Couple → Story → People → Location → Gallery
/// → Rsvp → Footer. `Gift` is commented out in that source file's own render,
/// so it is not ported here either.
library;
...
class RoyalWeddingTemplate extends StatelessWidget {
  const RoyalWeddingTemplate({super.key, required this.data});

  final WeddingWebsiteDetail data;

  @override
  Widget build(BuildContext context) {
    final theme = WeddingTemplateTheme.royal;
    return Container(
