...
class VendorAi {
  ...
  factory VendorAi.fromJson(Map<String, dynamic> j) => VendorAi(
    ...
    type: j['type'] ?? '',
    whyConsider: (j['why_consider'] as List<dynamic>?)
        ?.map((e) => e.toString())
        .toList() ??
        [],
  );
  ...
  Future<void> sendUserMessage(String text) async {
    ...
    try {
      ...
      if (data != null) {
        ...
        // ASSISTANT MESSAGE
        _messages.add(MessageItem(role: 'assistant', content: assistantContent));
  ...
  child: Text(ar.summary, style: const TextStyle(fontWeight: FontWeight.w600)),
  ),
  if (ar.message.isNotEmpty) Text(ar.message),
  if (ar.results.isNotEmpty) ...[
  const SizedBox(height: 8),
  // Brief list preview
  Text('Found ${ar.results.length} vendor(s). Tap to view cards.',
  ...
  const VendorCard({super.key, required this.vendor, this.onTap, this.onWishlist});
  ...
  const SizedBox(height: 4),
  Text('${vendor.type} • ${vendor.location}', style: const TextStyle(color: Colors.grey)),
  const SizedBox(height: 8),
  Wrap(
  spacing: 6,
  runSpacing: 6,
  ...
  @override
  Widget build(BuildContext context) {
  return Consumer<ChatProvider>(builder: (context, prov, _) {
  ...
  return Scaffold(
  appBar: AppBar(
  title: const Text('ShaadiAi Assistant',style: TextStyle(fontWeight: FontWeight.bold),),
  actions: [
  IconButton(
  icon: const Icon(Icons.refresh),
  onPressed: () => prov.startNewChat(existingSessionId: prov.sessionId),
  ...
  // },
    itemCount: prov.messages.length +
        (prov.isLoading ? 1 : 0) +      // typing indicator extra row
        (_shouldShowVendorCards(prov) ? 1 : 0),  // vendor cards extra row

    itemBuilder: (context, idx) {
      final vendorCardsIndex = prov.messages.length;
      final typingIndex =
          prov.messages.length + (_shouldShowVendorCards(prov) ? 1 : 0);

      // 1️⃣ Vendor Cards Section
      if (_shouldShowVendorCards(prov) && idx == vendorCardsIndex) {
        final last = prov.messages.last;
        final ar = last.content as AssistantResponse;
  ...
  final title = it['title'] ?? 'Session ${it['session_id'] ?? it['id']}';
  ...
  Align(alignment: Alignment.centerLeft, child: const Text('Why consider', style: TextStyle(fontWeight: FontWeight.bold))),
  const SizedBox(height: 8),
  Wrap(
  spacing: 8,
  children: v.whyConsider.map((s) => Chip(label: Text(s))).toList(),
  ),
